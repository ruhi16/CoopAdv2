...
class Ec15ThfundSpecificationComp extends Component
{
    use WithPagination;
    ...
    {
        return [
            'thfund_master_db_id' => 'nullable|integer|min:0',
    ...
    {
        $specs = Ec15ThfundSpecification::with(['masterDb'])
        ...
        return view('livewire.ec15-thfund-specification-comp', compact('specs'));
    }

    public function mount()
    {
        $this->masterDbs = Ec15ThfundMasterDb::where('is_active', true)->orderBy('name')->get();
    }
    ...
    public function store()
    {
        ...
        Ec15ThfundSpecification::updateOrCreate(['id' => $this->spec_id], array_merge($validated, [
    ...
    {
        $spec = Ec15ThfundSpecification::findOrFail($id);
        
        $this->spec_id = $id;
        $this->thfund_master_db_id = $spec->thfund_master_db_id ?? '';
        $this->name = $spec->name;
    ...
    {
        Ec15ThfundSpecification::find($id)->delete();
        $this->confirmingDelete = null;
