        ...
        <div class="p-4">
            ...
            <div class="flex justify-between mb-4 gap-2">
                <div class="flex gap-2">
                    ...
                    <select class="px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="financial_year">
                        <option value="">All Financial Years</option>
                        @foreach($availableFinancialYears as $year)
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-left font-medium text-gray-600">Fin. Year</th>
                            <th class="px-3 py-2 text-left font-medium text-gray-600">Join Date</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($members as $member)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 py-2">{{ $loop->iteration + ($members->currentPage() - 1) * $members->perPage() }}</td>
                                <td class="px-3 py-2">{{ $member->name }}</td>
                                <td class="px-3 py-2">{{ $member->memberDb->name ?? '-' }}</td>
                                <td class="px-3 py-2">{{ $member->financial_year }}</td>
                                <td class="px-3 py-2">{{ $member->join_date }}</td>
    ...
    @if ($isOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="relative bg-white rounded-lg shadow-lg w-full max-w-lg mx-4">
                <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                    <h5 class="font-semibold text-gray-700">{{ $member_id ? 'Edit Member' : 'Create Member' }}</h5>
                ...
                <div class="p-4">
                    <form>
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Member DB</label>
                                <select class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="member_db_id">
                                    <option value="">Select Member</option>
                                    @foreach($memberDbs as $db)
                        ...
                        <div class="grid grid-cols-2 gap-3 mt-3">
                            ...
                            <div>
                                ...
                                <select class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="financial_year">
                        ...
                        <div class="grid grid-cols-2 gap-3 mt-3">
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Join Date</label>
                                <input type="date" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="join_date">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Exit Date</label>
                                <input type="date" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="exit_date">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-3 mt-3">
                            <div>
                ...
                <div class="px-4 py-3 border-t border-gray-200 flex justify-end gap-2">
                    ...
                    <button class="px-3 py-1.5 bg-blue-600 text-white text-sm rounded hover:bg-blue-700" wire:click="store()">{{ $member_id ? 'Update' : 'Save' }}</button>
                </div>
            </div>
        </div>
    @endif
