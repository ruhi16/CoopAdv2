<div class="p-4">
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-700">THFUND Master</h3>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($masterDbs as $masterDb)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-3 py-2">
                                    @if($masterDb->thfund_operational_type)
                                        <span class="px-2 py-1 text-xs rounded {{ $masterDb->thfund_operational_type === 'deposit' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                            {{ ucfirst($masterDb->thfund_operational_type) }}
                                        </span>
                                    @else
                                        -
                                    @endif
                                </td>
                                <td class="px-3 py-2">{{ $masterDb->thfund_operational_amount ? number_format($masterDb->thfund_operational_amount, 2) : '-' }}</td>
                                <td class="px-3 py-2">{{ $masterDb->thfund_current_balance ? number_format($masterDb->thfund_current_balance, 2) : '-' }}</td>
                                <td class="px-3 py-2">
                                    @if($masterDb->start_at && $masterDb->end_at)
                                        {{ $masterDb->start_at }} to {{ $masterDb->end_at }}
                                        <br><span class="text-xs text-gray-500">{{ $masterDb->no_of_months }} months</span>
                                    @else
                                        -
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="11" class="px-3 py-4 text-center text-gray-500">No records found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                    ...
                    <form>
                        ...
                        <div class="grid grid-cols-2 gap-3 mt-3">
                            ...
                            <div>
                                ...
                                <select class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="thfund_operational_type">
                        ...
                        <div class="grid grid-cols-4 gap-3 mt-3">
                            <div>
                                ...
                                <input type="number" step="0.01" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="thfund_operational_amount" placeholder="0.00">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Operational Date</label>
                                <input type="date" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="thfund_operational_date">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Current Balance</label>
                                <input type="number" step="0.01" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="thfund_current_balance" placeholder="0.00">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">No. of Months</label>
                                <input type="number" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="no_of_months" placeholder="0">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-3 mt-3">
                            <div>
                                ...
                                <input type="date" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="start_at">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">End Date</label>
                                <input type="date" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="end_at">
                            </div>
                            <div>
