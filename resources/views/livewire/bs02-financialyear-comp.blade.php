<div class="p-4">
    <div class="bg-white rounded-lg shadow">
        <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-700">Financial Years</h3>
        </div>
        <div class="p-4">
            ...
            <div class="flex justify-between mb-4 gap-2">
                ...
                <button class="px-4 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700" wire:click="create()">Create Financial Year</button>
            </div>
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-left font-medium text-gray-600">End Date</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($financialyears as $fy)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 py-2">{{ $loop->iteration + ($financialyears->currentPage() - 1) * $financialyears->perPage() }}</td>
                                <td class="px-3 py-2">{{ $fy->name }}</td>
                                <td class="px-3 py-2">{{ $fy->start_date }}</td>
                                <td class="px-3 py-2">{{ $fy->end_date }}</td>
                                <td class="px-3 py-2">{{ $fy->order_index }}</td>
                                <td class="px-3 py-2">{{ $fy->status }}</td>
                                <td class="px-3 py-2">
                                    @if($fy->is_active)
                                ...
                                <td class="px-3 py-2">
                                    <button class="px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700 mr-1" wire:click="edit({{ $fy->id }})">Edit</button>
                                    @if ($confirmingDelete === $fy->id)
                                        <button class="px-2 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-700 mr-1" wire:click="delete({{ $fy->id }})">Confirm</button>
                                        <button class="px-2 py-1 bg-gray-500 text-white text-xs rounded hover:bg-gray-600" wire:click="cancelDelete()">Cancel</button>
                                    @else
                                        <button class="px-2 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-700" wire:click="confirmDelete({{ $fy->id }})">Delete</button>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="px-3 py-4 text-center text-gray-500">No financial years found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    @if ($isOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="relative bg-white rounded-lg shadow-lg w-full max-w-lg mx-4">
                <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                    <h5 class="font-semibold text-gray-700">{{ $financialyear_id ? 'Edit Financial Year' : 'Create Financial Year' }}</h5>
                    ...
                    <form>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="text" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="name" placeholder="e.g., 2025-2026">
                        ...
                        <div class="grid grid-cols-2 gap-3 mt-3">
                            <div>
                                ...
                                <input type="date" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="start_date">
                                @error('start_date') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">End Date</label>
                                <input type="date" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="end_date">
                ...
                <div class="px-4 py-3 border-t border-gray-200 flex justify-end gap-2">
                    ...
                    <button class="px-3 py-1.5 bg-blue-600 text-white text-sm rounded hover:bg-blue-700" wire:click="store()">{{ $financialyear_id ? 'Update' : 'Save' }}</button>
                </div>
            </div>
        </div>
    @endif
