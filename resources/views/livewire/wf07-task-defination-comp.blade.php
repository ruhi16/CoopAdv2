<div class="p-4">
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-700">Task Definitions</h3>
            <button class="px-4 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700" wire:click="create()">Create Task Definition</button>
        </div>
        <div class="p-4">
            <div class="flex justify-between mb-4 gap-2">
                <div class="flex gap-2">
                    ...
                    <select class="px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="task_category_filter">
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-left font-medium text-gray-600">Event</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($taskDefinations as $defination)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 py-2">{{ $loop->iteration + ($taskDefinations->currentPage() - 1) * $taskDefinations->perPage() }}</td>
                                <td class="px-3 py-2 font-medium">{{ $defination->name }}</td>
                                <td class="px-3 py-2">{{ $defination->taskCategory->name ?? '-' }}</td>
                                <td class="px-3 py-2">{{ $defination->taskEvent->name ?? '-' }}</td>
                                <td class="px-3 py-2">{{ $defination->description ?? '-' }}</td>
                                <td class="px-3 py-2">{{ $defination->school_id ?? '-' }}</td>
                                <td class="px-3 py-2">
                                    @if($defination->is_active)
                                ...
                                <td class="px-3 py-2">{{ $defination->remarks ?? '-' }}</td>
                                <td class="px-3 py-2">{{ $defination->created_at->format('d-m-Y') }}</td>
                                <td class="px-3 py-2">
                                    <button class="px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700 mr-1" wire:click="edit({{ $defination->id }})">Edit</button>
                                    @if ($confirmingDelete === $defination->id)
                                        <button class="px-2 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-700 mr-1" wire:click="delete({{ $defination->id }})">Confirm</button>
                                        <button class="px-2 py-1 bg-gray-500 text-white text-xs rounded hover:bg-gray-600" wire:click="cancelDelete()">Cancel</button>
                                    @else
                                        <button class="px-2 py-1 bg-red-500 text-white text-xs rounded hover:bg-red-600" wire:click="confirmDelete({{ $defination->id }})">Delete</button>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="10" class="px-3 py-4 text-center text-gray-500">No task definitions found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    @if($isOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
                <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                    <h3 class="text-lg font-semibold text-gray-700">{{ $task_defination_id ? 'Edit' : 'Create' }} Task Definition</h3>
                ...
                <div class="p-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="name" placeholder="Enter task name">
                    ...
                    <div class="mb-3">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Task Event <span class="text-red-500">*</span></label>
                        <select class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="task_event_id">
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        ...
                        <button class="px-4 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700" wire:click="store">{{ $task_defination_id ? 'Update' : 'Create' }}</button>
                    </div>
                </div>
            </div>
        </div>
