<div class="p-4">
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-700">Task Event Phases</h3>
            <button class="px-4 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700" wire:click="create()">Create Task Event Phase</button>
        </div>
        <div class="p-4">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($taskEventPhases as $phase)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 py-2">{{ $loop->iteration + ($taskEventPhases->currentPage() - 1) * $taskEventPhases->perPage() }}</td>
                                <td class="px-3 py-2 font-medium">{{ $phase->name }}</td>
                                <td class="px-3 py-2">{{ $phase->description ?? '-' }}</td>
                                <td class="px-3 py-2">{{ $phase->school_id ?? '-' }}</td>
                                <td class="px-3 py-2">
                                    @if($phase->is_active)
                                ...
                                <td class="px-3 py-2">{{ $phase->remarks ?? '-' }}</td>
                                <td class="px-3 py-2">{{ $phase->created_at->format('d-m-Y') }}</td>
                                <td class="px-3 py-2">
                                    <button class="px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700 mr-1" wire:click="edit({{ $phase->id }})">Edit</button>
                                    @if ($confirmingDelete === $phase->id)
                                        <button class="px-2 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-700 mr-1" wire:click="delete({{ $phase->id }})">Confirm</button>
                                        <button class="px-2 py-1 bg-gray-500 text-white text-xs rounded hover:bg-gray-600" wire:click="cancelDelete()">Cancel</button>
                                    @else
                                        <button class="px-2 py-1 bg-red-500 text-white text-xs rounded hover:bg-red-600" wire:click="confirmDelete({{ $phase->id }})">Delete</button>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="px-3 py-4 text-center text-gray-500">No task event phases found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    @if($isOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
                <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                    <h3 class="text-lg font-semibold text-gray-700">{{ $task_event_phase_id ? 'Edit' : 'Create' }} Task Event Phase</h3>
                ...
                <div class="p-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="name" placeholder="Enter phase name">
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        ...
                        <button class="px-4 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700" wire:click="store">{{ $task_event_phase_id ? 'Update' : 'Create' }}</button>
                    </div>
                </div>
            </div>
        </div>
