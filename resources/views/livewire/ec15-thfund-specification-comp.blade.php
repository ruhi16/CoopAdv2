<div class="p-4">
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-700">THFUND Specifications</h3>
                ...
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-left font-medium text-gray-600">Master DB</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($specs as $spec)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-3 py-2">{{ $spec->masterDb->name ?? '-' }}</td>
                                <td class="px-3 py-2">
                                    @switch($spec->status)
                                        @case('draft')
            ...
            <div class="relative bg-white rounded-lg shadow-lg w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
                ...
                <div class="p-4">
                    <form>
                        ...
                        <div class="grid grid-cols-2 gap-3 mt-3">
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Master DB</label>
                                <select class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="thfund_master_db_id">
                                    <option value="">Select Master DB</option>
                                    @foreach($masterDbs as $masterDb)
                                        <option value="{{ $masterDb->id }}">{{ $masterDb->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
