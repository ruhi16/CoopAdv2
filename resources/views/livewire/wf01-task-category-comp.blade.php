<div class="p-4">
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-700">Task Categories</h3>
            <button class="px-4 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700" wire:click="create()">Create Task Category</button>
        </div>
        <div class="p-4">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($taskCategories as $category)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 py-2">{{ $loop->iteration + ($taskCategories->currentPage() - 1) * $taskCategories->perPage() }}</td>
                                <td class="px-3 py-2 font-medium">{{ $category->name }}</td>
                                <td class="px-3 py-2">{{ $category->description ?? '-' }}</td>
                                <td class="px-3 py-2">{{ $category->school_id ?? '-' }}</td>
                                <td class="px-3 py-2">
                                ...
                                <td class="px-3 py-2">{{ $category->remarks ?? '-' }}</td>
                                <td class="px-3 py-2">{{ $category->created_at->format('d-m-Y') }}</td>
                                <td class="px-3 py-2">
                                    <button class="px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700 mr-1" wire:click="edit({{ $category->id }})">Edit</button>
                                    @if ($confirmingDelete === $category->id)
                                        <button class="px-2 py-1 bg-red-600 text-white text-xs rounded hover:bg-red-700 mr-1" wire:click="delete({{ $category->id }})">Confirm</button>
                                        <button class="px-2 py-1 bg-gray-500 text-white text-xs rounded hover:bg-gray-600" wire:click="cancelDelete()">Cancel</button>
                                    @else
                                        <button class="px-2 py-1 bg-red-500 text-white text-xs rounded hover:bg-red-600" wire:click="confirmDelete({{ $category->id }})">Delete</button>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="px-3 py-4 text-center text-gray-500">No task categories found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    @if($isOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
                <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                    <h3 class="text-lg font-semibold text-gray-700">{{ $task_category_id ? 'Edit' : 'Create' }} Task Category</h3>
                ...
                <div class="p-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-blue-500" wire:model="name" placeholder="Enter category name">
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        ...
                        <button class="px-4 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700" wire:click="store">{{ $task_category_id ? 'Update' : 'Create' }}</button>
                    </div>
                </div>
            </div>
        </div>
